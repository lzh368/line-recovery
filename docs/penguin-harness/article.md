...
![图 1：来电后输送带未恢复，人工与 Agent 的处理流程示意。](images/01-inspection-human-vs-agent.png)

图 1：来电后输送带未恢复，人工与 Agent 的处理流程示意。
...
![图 2：Penguin Harness 开发与优化产线巡检 Agent。](images/02-penguin-develops-inspection-agent.png)

图 2：Penguin Harness 开发与优化产线巡检 Agent。
...
![图 3：客户端下载页：按操作系统选择安装包。](images/03-01-download.png)

图 3：客户端下载页：按操作系统选择安装包。

安装完成后，打开客户端。在主界面左侧点击“模型库”，即可配置接下来要使用的模型。

![图 4：Penguin Harness 主界面，左侧为“模型库”入口。](images/04-02-home.png)

图 4：Penguin Harness 主界面，左侧为“模型库”入口。
...
在“模型库”中展开 DeepSeek 分组，打开 DeepSeek Flash 的模型配置，填入自己的 API Key，点击“确认”保存。通过官方服务接入时，API 地址为 [https://api.deepseek.com](https://api.deepseek.com)，模型 ID 为 deepseek-flash。下图标出了对应位置。

![图 5：DeepSeek Flash 模型配置：填写 API Key、API 地址和模型 ID。密钥已遮挡。](images/05-04-api-settings.png)

图 5：DeepSeek Flash 模型配置：填写 API Key、API 地址和模型 ID。密钥已遮挡。

### 2.3 选择 DeepSeek 模型

保存后，DeepSeek Flash 会显示在模型库中。下图中的“默认”标签表示它已设为默认模型；也可以在每次任务开始前单独选择模型。

![图 6：模型库中的 DeepSeek Flash；“默认”标签表示当前默认模型。](images/06-05-model-list.png)

图 6：模型库中的 DeepSeek Flash；“默认”标签表示当前默认模型。

点击左侧“新建对话”，在输入框右下方打开模型列表，选择 DeepSeek Flash。列表中的勾选标记表示本次任务将使用该模型。

![图 7：在输入框右下方选择本次任务使用的模型。](images/07-06-model-picker.png)

图 7：在输入框右下方选择本次任务使用的模型。
...
![图 8：选择项目文件夹后，点击“使用此目录”。](images/08-07-workspace.png)

图 8：选择项目文件夹后，点击“使用此目录”。

输入框下方的“技能”菜单列出了当前可用的技能，它们为特定任务提供操作说明。可以先熟悉这个入口，开发或评测时再选择相应技能；本次读取项目的简单任务无需额外选择。

![图 9：输入框下方的“技能”菜单，列出当前可用技能。](images/09-08-skills.png)

图 9：输入框下方的“技能”菜单，列出当前可用技能。
...
![图 10：选定工作区并输入指令后，点击右侧箭头发送。](images/10-09-send-task.png)

图 10：选定工作区并输入指令后，点击右侧箭头发送。

发送后，可以在对话中查看回复，并展开工具调用记录，了解它读取了哪些文件。得到与工作区内容相符的介绍后，就完成了首次交互。

![图 11：任务完成后的项目介绍与文件读取记录。](images/11-10-task-result.png)

图 11：任务完成后的项目介绍与文件读取记录。
...
第二章已经将 [line-recovery-starter 仓库](https://github.com/rank-Yu/line-recovery-starter)克隆到本地。这个仓库保留了原项目的业务需求、输入示例、数据契约和两个模拟设备 MCP 服务，尚未包含生成后的 app/。本章从这份材料出发，介绍开发、启动和结果核查步骤；读者可以用仓库自带的产线巡检示例数据包（lr_001）完成首次联调。
...
| 工作区材料 | 开发时的用途 |
|-|-|
| README.md | 业务需求、材料阅读顺序和应用交付要求 |
| examples/input/ | 示例数据包，包括运行记录、事件日志、状态快照、工位图片与操作说明 |
| contracts/ | 输入、设备状态、操作和报告的格式规范 |
| interfaces/ | 输送恢复与散热控制 MCP 服务，以及初始化和接入说明 |
...
| MCP 服务 | 用途 |
|-|-|
| power-control | 查询供电和输送带状态，在条件满足时启动输送带。 |
| cooling-control | 查询温度和风机状态，开启风机散热。 |
...
![图 12：选择 line-recovery-starter 目录作为开发工作区。](images/12-11-starter-workspace.png)

图 12：选择 line-recovery-starter 目录作为开发工作区。

打开输入框下方的“技能”菜单，选择 agent-initialization 技能。

![图 13：在技能菜单中选择 agent-initialization。](images/13-14-select-skill.png)

图 13：在技能菜单中选择 agent-initialization。
...
![图 14：确认工作区、技能和模型后，发送完整开发指令。](images/14-15-build-prompt.png)

图 14：确认工作区、技能和模型后，发送完整开发指令。

收到指令后，Penguin Harness 开始读取需求、接口文档和示例资料，再开发产线巡检 Agent 和配套网页。展开运行记录，可以查看具体的工具调用。

![图 15：Penguin Harness 开发过程中读取项目资料。](images/15-development-process.png)

图 15：Penguin Harness 开发过程中读取项目资料。

本例初版开发用时约 20 分钟，完成后给出了以下交付说明：

![图 16：原项目的初始交付说明：应用框架先交付，设备接口随后继续接通。](images/15-initial-delivery-cropped.png)

图 16：原项目的初始交付说明：应用框架先交付，设备接口随后继续接通。
...
开发完成后，按照 app/README.md 中的说明配置并启动应用，再打开启动日志中的本地地址，即可查看网页。
...
![图 17：产线恢复助手初始界面。](images/16-app-initial.png)
...
![图 18：产线恢复助手首页，红框为资料上传入口。](images/16-16-app-upload-entry.png)

图 18：产线恢复助手首页，红框为资料上传入口。
...
![图 19：查看案例中的工位图片。](images/17-lr_101-03-photo.png)

图 19：查看案例中的工位图片。

接着切换到“时序数据”页签，查看供电和带速的变化。如下图所示，供电已经恢复，但带速仍为零，产出计数也没有增加。

![图 20：供电已经恢复，但带速仍为零。](images/18-lr_101-02-telemetry.png)

图 20：供电已经恢复，但带速仍为零。

切换到“诊断与证据”页签，点击“开始诊断”；已有报告时，按钮显示为“重新诊断”。完成后，页面会展示 Agent 的分析结论和对应证据。

![图 21：Agent 的停机分析与对应证据。](images/19-lr_101-04-diagnosis.png)

图 21：Agent 的停机分析与对应证据。
...
![图 22：恢复输送后，带速和产出计数发生变化。](images/20-lr_101-06-feedback.png)

图 22：恢复输送后，带速和产出计数发生变化。
...
![图 23：Penguin Harness 评测与优化 Agent 应用。](images/21-penguin-optimization-simple.png)

图 23：Penguin Harness 评测与优化 Agent 应用。
...
![图 24：选择 line-recovery 根目录，点击“使用此目录”。](images/22-21-optimization-workspace.png)

图 24：选择 line-recovery 根目录，点击“使用此目录”。
...
![图 25：选中 agent-evaluation 和 agent-optimization，菜单中两项均显示勾选。](images/23-22-optimization-skills.png)

图 25：选中 agent-evaluation 和 agent-optimization，菜单中两项均显示勾选。
...
![图 26：填好优化指令后的待发送状态；红框为发送按钮，本次截图未启动任务。](images/24-23-optimization-prompt.png)

图 26：填好优化指令后的待发送状态；红框为发送按钮，本次截图未启动任务。

点击图中红框标出的发送按钮。展开运行记录，可以看到 Penguin Harness 读取评测说明和相关技能，准备评测与优化任务，如下图所示。

![图 27：评测与优化任务启动后的工具调用记录。](images/27-optimization-process.png)

图 27：评测与优化任务启动后的工具调用记录。
...
![图 28：优化前后成绩。训练集来自 round1；七例测试来自 gate-set-7，先取每例历史运行均分，再对七例求平均，各案例和版本的运行次数不完全相同。](images/25-test-score-comparison.png)

图 28：优化前后成绩。训练集来自 round1；七例测试来自 gate-set-7，先取每例历史运行均分，再对七例求平均，各案例和版本的运行次数不完全相同。
